...
<script>
    ...
    const LGULayerPicker = L.Control.extend({
        ...
        onAdd: function () {
            const layers = [
                ...
                { key: 'satellite', label: 'Satellite', icon: 'bi-globe',    layer: tileSatellite },
                { key: 'hybrid',    label: 'Hybrid',    icon: 'bi-layers',   layer: tileHybrid    },
                { key: 'terrain',   label: 'Terrain',   icon: 'bi-triangle', layer: tileTerrain   },
            ];

            const wrap = L.DomUtil.create('div');
            wrap.style.cssText = 'position:relative; display:flex; flex-direction:column; align-items:flex-end;';
            ...
            layers.forEach(({ key, label, icon, layer }) => {
                const btn = L.DomUtil.create('button', 'lgu-layer-opt' + (key === 'road' ? ' active' : ''), popup);
                btn.innerHTML = `
                    <span class="lgu-opt-icon"><i class="bi ${icon}"></i></span>
                    <span>${label}</span>
                    <span class="lgu-opt-radio">${key === 'road' ? '<span style="width:4px;height:4px;border-radius:50%;background:#fff;display:block;"></span>' : ''}</span>`;

                L.DomEvent.on(btn, 'click', function () {
                    if (activeKey === key) return;
                    layers.forEach(l => { if (map.hasLayer(l.layer)) map.removeLayer(l.layer); });
                    layer.addTo(map);
                    popup.querySelectorAll('.lgu-layer-opt').forEach(b => {
                        b.classList.remove('active');
                        b.querySelector('.lgu-opt-radio').innerHTML = '';
                    });
                    btn.classList.add('active');
                    btn.querySelector('.lgu-opt-radio').innerHTML = '<span style="width:4px;height:4px;border-radius:50%;background:#fff;display:block;"></span>';
                    activeKey = key;
                    // Update FAB icon to reflect current layer
                    fab.querySelector('i').className = 'bi ' + icon;
                });
            });

            // FAB toggle button
            const fab = L.DomUtil.create('button', 'lgu-layer-fab', wrap);
    ...
    const QC_BOUNDARY_COORDS = [
        ...
        [121.1099963, 14.7651342], [121.1095933, 14.7646242] // closing point
    ];

    // Convert [lng, lat] pairs to Leaflet [lat, lng] format
    const qcBoundaryLatLngs = QC_BOUNDARY_COORDS.map(c => [c[1], c[0]]);

    // Render the QC boundary polygon on the map
    const qcBoundaryLayer = L.polygon(qcBoundaryLatLngs, {
        color:       '#1a237e',   // LGU blue border
        weight:      3,
        opacity:     1,
        dashArray:   '8, 4',
        fillColor:   '#1a237e',
...
function checkSpatialCompliance(lat, lng, clickedFeature) {
    ...
    // QC City Boundary toggle
    document.getElementById('toggleQCBoundary').addEventListener('change', function() {
        if (this.checked) {
