// Translated strings and server data passed from PHP (see window.MAP_CONFIG in map.php)
    ...
    const allParcelsData = config.allParcels;
    const targetAppId = config.targetAppId;
    const appLat = parseFloat(config.appLat);
    const appLng = parseFloat(config.appLng);
    const dbLot = config.dbLot;
    const dbBlock = config.dbBlock;
...
function checkSpatialCompliance(lat, lng, clickedFeature) {
    ...
    // Auto-locate logic
    if (config.selectedParcel) {
        const sGeo = JSON.parse(config.selectedParcel.geoRaw);
        L.geoJSON(sGeo, { style: { color: '#ffd600', weight: 5, fillOpacity: 0.6 } }).addTo(map);
        map.fitBounds(L.geoJSON(sGeo).getBounds());
        checkSpatialCompliance(turf.center(sGeo).geometry.coordinates[1], turf.center(sGeo).geometry.coordinates[0], {properties: {id: config.selectedParcel.id, zone: config.selectedParcel.zone, lot: config.selectedParcel.lot, block: config.selectedParcel.block}});
    } else if (appLat && appLng) {
