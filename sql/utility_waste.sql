-- ============================================================
-- WASTE MANAGEMENT MODULE — Database Schema
-- utility_waste.sql
-- ============================================================
...
-- ── 2. WASTE ROUTE STOPS (ETA collection points) ───────────
CREATE TABLE IF NOT EXISTS waste_route_stops (
    ...
    travel_min     INT          NOT NULL DEFAULT 15 COMMENT 'Minutes from previous stop',
    service_min    INT          NOT NULL DEFAULT 10 COMMENT 'Service time at this stop',
...
CREATE TABLE IF NOT EXISTS waste_schedules (
    ...
    status       ENUM('Active','Suspended','Completed','Missed') DEFAULT 'Active',
    created_at   TIMESTAMP    DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (truck_id)  REFERENCES waste_trucks(id)  ON DELETE SET NULL,
    FOREIGN KEY (route_id)  REFERENCES waste_routes(id)  ON DELETE SET NULL
);
...
-- ── 8. WASTE COMPLIANCE AUDITS ───────────────────────────────
CREATE TABLE IF NOT EXISTS waste_compliance (
    id               INT AUTO_INCREMENT PRIMARY KEY,
    zone_name        VARCHAR(100) NOT NULL,
    barangay         VARCHAR(100),
    audit_date       DATE         NOT NULL,
    compliance_rate  DECIMAL(5,2) DEFAULT 0 COMMENT 'Percentage 0-100',
    audited_by       VARCHAR(100),
    notes            TEXT,
    created_at       TIMESTAMP    DEFAULT CURRENT_TIMESTAMP
);

-- ── 9. WASTE NOTIFICATIONS ───────────────────────────────────
...
SELECT
    (SELECT COUNT(*) FROM waste_collection_records WHERE MONTH(date_collected) = MONTH(CURDATE()) AND YEAR(date_collected) = YEAR(CURDATE())) AS monthly_collections,
    (SELECT COALESCE(SUM(volume_kg),0) FROM waste_collection_records WHERE MONTH(date_collected) = MONTH(CURDATE())) AS monthly_volume_kg,
    ...
    (SELECT COALESCE(AVG(compliance_rate),0) FROM waste_compliance WHERE MONTH(audit_date) = MONTH(CURDATE())) AS avg_compliance_rate;

SET FOREIGN_KEY_CHECKS = 1;

-- ── SEED: WASTE ROUTES ───────────────────────────────────────
INSERT IGNORE INTO waste_routes (id, route_name, color_hex, district, coverage, start_time) VALUES
...
INSERT IGNORE INTO waste_route_stops (route_id, stop_order, barangay_name, latitude, longitude, travel_min, service_min) VALUES
...
-- ── SEED: WASTE TRUCKS ───────────────────────────────────────
INSERT IGNORE INTO waste_trucks (truck_id, plate_number, driver_name, route_id, capacity_kg, status) VALUES
...
INSERT IGNORE INTO waste_facilities (facility_name, facility_type, location, latitude, longitude, capacity_tons, current_load_tons, status) VALUES
('QC Material Recovery Facility - Batasan', 'MRF', 'Batasan Hills, Quezon City', 14.6940, 121.1100, 500, 310, 'Operational'),
('Fairview Composting Site', 'Composting', 'Fairview, Quezon City', 14.7300, 121.0550, 200, 80, 'Operational'),
('Novaliches Transfer Station', 'Transfer Station', 'Novaliches, Quezon City', 14.7250, 121.0200, 800, 620, 'Operational');
