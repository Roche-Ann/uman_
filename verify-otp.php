<?php
// verify-otp.php - Complete OTP Verification
require_once 'includes/auth.php';

// If already fully logged in, go to dashboard
if (isLoggedIn()) {
    header('Location: ' . (isEmployee() ? 'utilities_dashboard.php' : 'citizen.php'));
    exit();
}

// Check if we have a pending login session
if (!isset($_SESSION['pending_login']) || empty($_SESSION['pending_login'])) {
...
$userId = $pending['id'] ?? 0;
$fullName = $pending['name'] ?? 'User';
$email = $pending['email'] ?? '';
$userType = $pending['role'] ?? 'citizen';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($otp) || strlen($otp) !== 6 || !ctype_digit($otp)) {
        ...
        if (!$otpRecord) {
            ...
            $expires = new DateTime($otpRecord['expires_at'], new DateTimeZone('UTC'));
            
            if ($now > $expires) {
                $error = 'OTP has expired. Please request a new one.';
                // Mark as used so it can't be reused
                $pdo->prepare("UPDATE otps SET used = 1 WHERE id = ?")->execute([$otpRecord['id']]);
            } else {
                // Verify the OTP hash
                if (password_verify($otp, $otpRecord['otp_hash'])) {
                    ...
                    // Optionally, increment failed attempts (not implemented here)
                }
            }
        }
    }
...
<head>
    ...
    <style>
        /* Same styling as login.php – copy the styles from login.php */
        :root {
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; }
        ...
        .civic-navigation, .login-section, .civic-footer { position: relative; z-index: 1; }
        /* Navigation – copy exactly from login.php for consistency */
        .civic-navigation {
            position: fixed;
            top: 0;
            width: 100%;
        ...
        .civic-navigation.scrolled { padding: 0.75rem 0; box-shadow: var(--shadow-persistent); }
        ...
        .logo-marker { background: none !important; border-radius: 0 !important; }
        ...
        .login-section { flex: 1; display: flex; align-items: center; justify-content: center; padding: 120px 1.5rem 60px; min-height: 100vh; }
        .login-card { background: rgba(255, 255, 255, 0.795); border-radius: var(--radius-modern); box-shadow: var(--shadow-elevated); padding: 2.5rem; width: 100%; max-width: 460px; border: 1px solid rgba(255, 255, 255, 0.3); animation: slideDown 0.7s cubic-bezier(0.34, 1.56, 0.64, 1); opacity: 0; animation-fill-mode: forwards; }
        ...
        .login-header .logo-only { width: 80px; height: 80px; margin-bottom: 1rem; }
        .login-header h2 { font-family: var(--font-heading); font-size: 2rem; font-weight: 700; background: linear-gradient(135deg, var(--civic-sapphire), var(--utility-teal)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; margin-bottom: 0.5rem; }
        .login-header p { color: rgba(47, 72, 88, 0.7); font-size: 0.95rem; }
        ...
        .input-box input { width: 100%; padding: 0.85rem 1rem; border: 2px solid rgba(11, 61, 145, 0.1); border-radius: var(--radius-soft); font-family: var(--font-primary); font-size: 1rem; transition: var(--transition-smooth); background: white; }
        ...
        .link { color: var(--civic-sapphire); text-decoration: none; font-weight: 600; transition: var(--transition-smooth); }
    ...
    <footer class="civic-footer">
        <div class="footer-grid">
            <div class="footer-brand">
                ...
                <p class="footer-description">We streamline utility operations and provide easy access to monitor your consumption and service requests.</p>
            </div>
            <div>
        ...
        <div class="footer-bottom">
            <div class="footer-copyright">© 2026 Utilities Management System · All Rights Reserved</div>
        </div>
    </footer>
        ...
        if (mobileMenuBtn && navLinksList) {
            mobileMenuBtn.addEventListener('click', () => {
                ...
                if (navLinksList.classList.contains('active')) {
                    ...
                    if (window.innerWidth <= 992 && mobileActionsGroup) mobileActionsGroup.style.display = 'flex';
                } else {
